...
class World(object):
	...
	def AlieninNeight(self,org):
		positions = self.getNeighboringPositions(org.position)
		AlieninNeigh = False
		for pos in positions:
			organism = self.getOrganismFromPosition(pos)
			if organism.__class__.__name__ == "Alien":
				AlieninNeigh = True
				break
			Action(ActionEnum.A_STOP, org.position, 4, self)
		return AlieninNeigh
	...
	def makeTurn(self):
		...
		for org in self.organisms:
			if self.positionOnBoard(org.position) or org.__class__.__name__ == 'Turtle':
				if self.AlieninNeight(org):#blokada ruchu
					stopped.append(org)
					self.makeMove(Action(ActionEnum.A_STOP, org.position, 4, org))
				else:
					actions = org.move()
					for a in actions:
						self.makeMove(a)
					...
					if self.positionOnBoard(org.position):
						...
						for a in actions:
							self.makeMove(a)
						actions = []
		self.organisms = [o for o in self.organisms if self.positionOnBoard(o.position) or o.__class__.__name__ == 'Turtle']
		for o in self.organisms:
			if o not in stopped:#blokada starzenia i dostawania mocy
				o.liveLength -= 1
				o.power += 1
			if o.liveLength < 1:
	...
	def makeMove(self, action):
		...
		elif action.action == ActionEnum.A_OUT:
			action.organism.position = action.position
